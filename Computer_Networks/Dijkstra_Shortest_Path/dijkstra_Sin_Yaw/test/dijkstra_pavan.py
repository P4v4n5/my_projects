...
class PathFinder(netEmulator):
    ...
    def main(self):
        ...
        if sys.argv[2] not in router_list or sys.argv[3] not in router_list:
            self.logger.error('Invalid Router name!! Please provide with the valid router names')
            sys.exit(1)
        ...
        self.logger.info('Total number of routers present in this net are ---> {}'.format(len(self.routers)))
    ...
    def find_shortest_path(self, start_node, end_node):
        if start_node == end_node:
            self.logger.warning("*-*-*-*-*-*-*-*---Start node and the End node are same!---*-*-*-*-*-*-*-*")
            return [start_node], 0
