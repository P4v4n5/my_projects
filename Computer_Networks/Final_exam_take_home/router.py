...
class Router:
    ...
    def send_data(self, dest, data):
        ...
        if dest in self.fib:
            ...
            self.logger.info(f"Router %s: Sending packet to %s via %s with cost: %s", self.name, dest, next_hop_name,
                             cost)
            ...
            self.logger.info(
                f"Source router ---> %s | Target router ---> %s. There is no FIB entry for %s. Hence, the packet got dropped.",
                self.name, dest, dest)
